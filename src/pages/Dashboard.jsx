        ...
        <div>
            ...
            <div className="grid-3" style={{ marginBottom: 28 }}>
                <div className="card" style={{ borderLeft: '4px solid var(--primary)' }}>
                    <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 8, fontWeight: 600 }}>TOTAL TRAVELS</div>
                    <div style={{ fontSize: 32, fontWeight: 800, color: 'var(--text)' }}>{travels.length}</div>
                </div>
                <div className="card" style={{ borderLeft: '4px solid var(--green)' }}>
                    <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 8, fontWeight: 600 }}>🟢 ONLINE NOW</div>
                    <div style={{ fontSize: 32, fontWeight: 800, color: 'var(--green)' }}>{online.length}</div>
                </div>
                <div className="card" style={{ borderLeft: '4px solid var(--border)' }}>
                    <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 8, fontWeight: 600 }}>⚫ OFFLINE</div>
                    <div style={{ fontSize: 32, fontWeight: 800, color: 'var(--text-muted)' }}>{offline.length}</div>
                </div>
            </div>
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead>
                                    <tr style={{ borderBottom: '1px solid var(--border)', background: 'rgba(255,255,255,0.02)' }}>
                                        {['Travel Name', 'Driver', 'Status', 'Last Seen', ''].map(h => (
                                            <th key={h} style={{
                                                padding: '16px',
                                                textAlign: 'left',
                                                fontSize: 12,
                                ...
                                <tbody>
                                    {travels.map(t => (
                                        ...
                                        >
                                            <td style={{ padding: '16px', fontWeight: 600, whiteSpace: 'nowrap' }}>{t.name}</td>
                                            <td style={{ padding: '16px', color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{t.driverName}</td>
                                            <td style={{ padding: '16px', whiteSpace: 'nowrap' }}>
                                                <span className={`badge badge-${t.isOnline ? 'online' : 'offline'}`}>
                                                    {t.isOnline ? '🟢 Online' : '⚫ Offline'}
                                                </span>
                                            </td>
                                            <td style={{ padding: '16px', color: 'var(--text-muted)', fontSize: 13, whiteSpace: 'nowrap' }}>{formatTime(t.lastSeen)}</td>
        ...
        <div className="card" style={{
            ...
            {travel.location && (
                <div style={{ textAlign: 'right', fontSize: 12, color: 'var(--text-muted)' }}>
                    <div>📍 {travel.location.latitude?.toFixed(4)}, {travel.location.longitude?.toFixed(4)}</div>
                    <div>🚀 {travel.location.speed?.toFixed(0) || 0} km/h</div>
                    <div>Updated: {formatTime(travel.location.timestamp)}</div>
                </div>
            )}
        </div>
    );
